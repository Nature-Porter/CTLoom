...
from ctloom.config import ConfigError, load_config
from ctloom.hooks import HookConfigError, HookEngine, load_hooks
from ctloom.permissions import PermissionMode


def main() -> None:
    # 先确保 .ctloom/ 目录存在，否则下面写 debug.log 会因目录不存在而崩溃
    Path(".ctloom").mkdir(parents=True, exist_ok=True)
    logging.basicConfig(
        level=logging.INFO,
        format="%(asctime)s %(name)s %(message)s",
        filename=".ctloom/debug.log",
        filemode="w",
    )

    parser = argparse.ArgumentParser(prog="ctloom", description="MewCode AI coding assistant")
    parser.add_argument(
        "--mode",
    ...
    if args.remote:
        from ctloom.remote import RemoteServer
    ...
    from ctloom.app import MewCodeApp
    from ctloom.driver import NoAltScreenDriver
...
async def _run_prompt(config, permission_mode, hook_engine, prompt: str, output_format: str = "text") -> None:
    ...
    from ctloom.client import create_client, resolve_context_window
    from ctloom.conversation import ConversationManager
    from ctloom.memory.instructions import load_instructions
