...
def _session_path(ctloom_dir: Path) -> Path:
    return ctloom_dir / SESSION_FILENAME
...
def load_worktree_session(ctloom_dir: Path) -> WorktreeSession | None:
    path = _session_path(ctloom_dir)
    if not path.exists():
        return None
    try:
