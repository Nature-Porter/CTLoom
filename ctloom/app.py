...
from ctloom.commands import (
    CommandContext,
    CommandRegistry,
    complete,
    parse_command,
)
from ctloom.commands.completion import CompletionPopup
from ctloom.commands.handlers import register_all_commands
from ctloom.config import MCPServerConfig, ProviderConfig
from ctloom.hooks import HookContext, HookEngine, load_hooks
from ctloom.conversation import ConversationManager, Message
from ctloom.mcp import ConnectResult, MCPManager
from ctloom.memory import (
    MemoryManager,
    Session,
    SessionManager,
...
from ctloom.agents.notification import inject_task_notifications
from ctloom.commands.handlers.tasks import create_tasks_command
from ctloom.skills.executor import SkillExecutor
from ctloom.skills.loader import SkillLoader
from ctloom.commands.handlers.skill_register import register_skill_commands
from rich.text import Text as RichText
from textual.theme import Theme
from ctloom.cache import FileCache
from ctloom.tools import ToolRegistry, create_default_registry
from ctloom.tools.agent_tool import AgentTool
from ctloom.tools.ask_user import AskUserEvent, AskUserTool
from ctloom.tools.impl.tool_search import ToolSearchTool
from ctloom.tools.load_skill import LoadSkill
from ctloom.worktree.cleanup import start_stale_cleanup_task
from ctloom.worktree.manager import WorktreeManager
from ctloom.commands.handlers.worktree import create_worktree_command
from ctloom.teammate_tree import TeammateTree
...
_SKIP_DIRS = {".git", "node_modules", ".venv", "__pycache__", ".ctloom", "build", ".gradle"}
...
class ChatInput(TextArea):
    ...
    def load_history(self, work_dir: str) -> None:
        self._history_file = Path(work_dir) / ".ctloom" / "history"
...
class MewCodeApp(App):
    ...
    def __init__(
        ...
        from ctloom.config import SandboxAppConfig
    ...
    def on_mount(self) -> None:
        self.register_theme(_CTLOOM_THEME)
        self.theme = "ctloom"
        if len(self.providers) == 1:
    ...
    def _select_provider(self, provider: ProviderConfig) -> None:
        ...
        checker = PermissionChecker(
            ...
            rule_engine=RuleEngine(
                user_rules_path=home / ".ctloom" / "permissions.yaml",
                project_rules_path=Path(work_dir) / ".ctloom" / "permissions.yaml",
                local_rules_path=Path(work_dir) / ".ctloom" / "permissions.local.yaml",
            ),
        ...
        if self._sandbox_cfg.enabled:
            from ctloom.sandbox import SandboxConfig, create_sandbox
            os_sandbox = create_sandbox()
            if os_sandbox and os_sandbox.available():
                sandbox_config = SandboxConfig(
                    allow_write=[work_dir, "/tmp"],
                    deny_write=[
                        f"{work_dir}/.ctloom/config.yaml",
                        f"{work_dir}/.ctloom/permissions.local.yaml",
                    ],
        ...
        from ctloom.filehistory import FileHistory
        ...
        from ctloom.tools.exit_plan_mode import ExitPlanModeTool
        ...
        # --- Worktree 系统初始化 ---
        from ctloom.config import WorktreeConfig
        ...
        from ctloom.tools.enter_worktree import EnterWorktreeTool
        from ctloom.tools.exit_worktree import ExitWorktreeTool
        ...
        # --- Agent 团队系统初始化 ---
        from ctloom.teams.manager import TeamManager
        from ctloom.tools.team_create import TeamCreateTool
        from ctloom.tools.team_delete import TeamDeleteTool
        ...
        from ctloom.commands.handlers.trace import create_trace_command
        ...
        from ctloom.tools.synthetic_output import SyntheticOutputTool
    ...
    async def _prefetch_relevant_memories(self, query: str) -> str:
        ...
        async def selector(system_prompt: str, user_message: str) -> str:
            from ctloom.tools.base import StreamEnd, TextDelta
    ...
    async def _show_plan_approval(self) -> None:
        from ctloom.plan_dialog import InlinePlanWidget
    ...
    def on_inline_plan_widget_responded(
        ...
        from ctloom.plan_dialog import InlinePlanWidget, PlanChoice
        from ctloom.prompts import build_plan_mode_exit_reminder
    ...
    async def _handle_askuser(self, event: AskUserEvent) -> None:
        from ctloom.askuser_dialog import InlineAskUserWidget
    ...
    async def _handle_permission_request(self, request: PermissionRequest) -> None:
        from ctloom.permission_dialog import InlinePermissionWidget
