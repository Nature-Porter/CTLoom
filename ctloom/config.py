...
def load_config(path: Path | None = None) -> AppConfig:
    ...
    home = Path.home()
    candidates = [
        home / ".ctloom" / "config.yaml",
        cwd / ".ctloom" / "config.yaml",
        cwd / ".ctloom" / "config.local.yaml",
    ]
    ...
    if merged is None:
        raise ConfigError(
            "No config file found. Expected .ctloom/config.yaml "
            "in project or ~/.ctloom/config.yaml"
        )
    return merged
