...
from ctloom.conversation import ConversationManager, ToolResultBlock, ToolUseBlock
from ctloom.conversation import ThinkingBlock as ConvThinkingBlock
...
from ctloom.hooks import HookContext, HookEngine, ToolRejectedError
from ctloom.hooks.engine import HookNotification
...
class Agent:
    ...
    def _transcript_path(self) -> str:
        if self.session_id:
            return str(Path(self.work_dir) / ".ctloom" / "sessions" / f"{self.session_id}.jsonl")
        return ""
    ...
    def _get_plan_path(self) -> Path:
        ...
        plans_dir = Path(self.work_dir) / ".ctloom" / "plans"
        ...
        if self.permission_checker:
            ...
            if decision.effect == "ask":
                ...
                if response == PermissionResponse.ALLOW_ALWAYS:
                    from ctloom.permissions.rules import Rule, extract_content
